...
public class CarTypeNameService
{
    public async Task<List<CarTypeName>> GetAllAsync(CancellationToken ct = default)
    {
        await using var db = new RailwayContext();
        return await db.CarTypeNames
            .AsNoTracking()
            .OrderBy(x => x.Name)
            .ToListAsync(ct);
    }
    ...
    public async Task<(bool Ok, string Error)> AddAsync(string name, CancellationToken ct = default)
    {
        ...
        if (await db.CarTypeNames.AnyAsync(x => x.Name == name, ct))
            return (false, "Така назва вже існує");

        db.CarTypeNames.Add(new CarTypeName { Name = name });
    ...
    public async Task<(bool Ok, string Error)> UpdateAsync(int id, string name, CancellationToken ct = default)
    {
        ...
        var entity = await db.CarTypeNames.FirstOrDefaultAsync(x => x.Id == id, ct);
        if (entity == null)
            return (false, "Не знайдено");

        if (await db.CarTypeNames.AnyAsync(x => x.Name == name && x.Id != id, ct))
            return (false, "Така назва вже існує");
